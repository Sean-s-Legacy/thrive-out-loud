...
export const createAccount = async (payload: Partial <FirestoreUserData>) => {
  try {
    const { user_chosen_name, user_date_of_birth, user_pronouns, user_email, user_location, user_gender_identity, user_sexual_orientation, user_ethnicity, user_language, user_industry, user_focus_area, user_role, id} = payload;
