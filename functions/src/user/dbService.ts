...
export const updateMenteeAccount = async (
    payload: MenteeUpdatePayload
) => {
    try {
        const {user_email, user_pswd, user_name_first, user_name_last} = payload;
        const updateUserDocRef: FirebaseFirestore.DocumentReference = db.collection(COLLECTION.USERS).doc()
    ...
    await updateUserDocRef.set(user);
